...
//  
//
//  Created by Mongy on 10/02/2024.
//
...
extension Blog.Category.Update: Content {}
extension Blog.Category.Patch: Content {}
...
struct BlogCategoryApiController: ModelController {
    ...
    func patchApi(
        _ req: Request
    ) async throws -> Response {let model = try await findBy(identifier(req), on: req.db)
